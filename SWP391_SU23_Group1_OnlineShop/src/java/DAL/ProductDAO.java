...
public class ProductDAO extends DBContext {
    ...
    public List<Product> getProductPopularTopX(int x) {
        ...
        try {
            ...
            String sql = "SELECT TOP(?) p.[id]\n"
                    ...
                    + "                    FROM [dbo].[products] p  inner join [dbo].categories c\n"
                    + "                    ON p.category_id = c.id\n"
                    + "                    , (SELECT od.pid, COUNT(od.pid) as 'totalSold'\n"
                    + "						FROM (SELECT  od.[product_id ] as 'pid' \n"
                    + "FROM [dbo].[order_detail] od  inner join [dbo].[orders] o \n"
                    + "on o.id = od.order_id\n"
                    + "where o.status = 1) as od GROUP BY od.pid) as sold\n"
    ...
    public List<Product> getProductTopXSoldByCategoryID(int id) {
        List<Product> list = new ArrayList<>();
        ...
        try {
            ...
            String sql = "  SELECT p.[id]\n"
                    ...
                    + "         ,c.[update_at] as 'cUpdateAt'\n"
                    + "     FROM [dbo].[products] p  inner join [dbo].categories c\n"
                    + "     ON p.category_id = c.id, (SELECT TOP(5) p.[id] as 'product_id', COUNT(od.product_id) 'sold'\n"
                    + "								FROM [dbo].[products] p left join [dbo].[order_detail] od\n"
                    + "								  on p.id = od.product_id\n"
                    + "								  GROUP BY p.id, od.product_id,  p.created_at, p.category_id\n"
                    + "								  HAVING p.category_id = ?\n"
                    + "								  order by sold DESC, p.created_at DESC) as sold\n"
                    + "								  where p.id IN(sold.product_id)";
